...
<head>
    ...
    <style>
        * { font-family: 'Alexandria', sans-serif; }
        ...
        /* Calm Color Palette - ألوان هادية */
        :root {
            --color-primary: #6B9AC4;      /* أزرق هادي */
            --color-secondary: #8BA888;    /* أخضر ناعم */
            --color-accent: #D4A574;       /* برتقالي فاتح */
            --color-warning: #E8A87C;      /* برتقالي دافئ */
            --color-danger: #C97C7C;       /* أحمر هادي */
            --color-success: #88B2AC;      /* أخضر مائي */
            --color-info: #9DB4C0;         /* رمادي مزرق */
...
<!-- Login Screen -->
<div id="loginScreen" class="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-900 dark:to-slate-800 p-4">
    <div class="bg-white dark:bg-slate-800 p-6 sm:p-8 rounded-3xl shadow-2xl w-full max-w-md border-2 border-slate-200 dark:border-slate-700">
        <div class="text-center mb-6">
            <h1 class="text-3xl sm:text-4xl font-black text-slate-800 dark:text-white mb-2">
                B36 System
            </h1>
        ...
        <form onsubmit="event.preventDefault(); login();" class="space-y-4 mb-6">
            <input type="text" id="username" placeholder="اسم المستخدم" class="w-full p-3 border-2 border-slate-300 dark:border-slate-600 rounded-xl text-center font-bold focus:border-[#6B9AC4] outline-none bg-white dark:bg-slate-700 text-slate-800 dark:text-white transition">
            <input type="password" id="password" placeholder="كلمة المرور" class="w-full p-3 border-2 border-slate-300 dark:border-slate-600 rounded-xl text-center font-bold focus:border-[#6B9AC4] outline-none bg-white dark:bg-slate-700 text-slate-800 dark:text-white transition">
        ...
        <div class="border-t border-slate-200 dark:border-slate-700 pt-4">
            ...
            <div class="text-center space-y-1">
                <p class="text-sm font-bold text-black dark:text-slate-300">عبدالرحمن المالكي</p>
                <p class="text-sm font-bold text-black dark:text-slate-300">عبدالعزيز الأحمدي</p>
                <p class="text-sm font-bold text-black dark:text-slate-300">عبدالعزيز الذبياني</p>
            </div>
            <div class="flex items-center justify-center gap-2 mt-3">
                <span class="text-black dark:text-slate-400 text-xs font-bold">Made with</span>
                <span class="text-red-500 text-lg animate-heartbeat">❤️</span>
...
<div id="mainApp" class="hidden min-h-screen">
    ...
    <button id="hamburgerBtn" class="lg:hidden fixed top-4 right-4 z-50 bg-white dark:bg-slate-800 p-3 rounded-xl shadow-lg" onclick="toggleSidebar()">
        <i class="ph ph-list text-2xl text-slate-800 dark:text-white"></i>
    </button>
    
    <!-- Sidebar Overlay for Mobile -->
    <div id="sidebarOverlay" class="lg:hidden hidden fixed inset-0 bg-black/50 z-30" onclick="toggleSidebar()"></div>
    
    <!-- Sidebar -->
    <aside id="sidebar" class="drawer-closed w-64 bg-white dark:bg-slate-800 border-l dark:border-slate-700 fixed h-full flex flex-col shadow-lg z-40 right-0 lg:right-auto">
        ...
        <nav class="flex-1 p-4 space-y-2 overflow-y-auto">
            <button onclick="showView('dashboard')" class="nav-btn w-full flex items-center gap-3 p-3 rounded-xl font-bold text-sm hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 transition">
                <i class="ph ph-squares-four text-xl text-[#6B9AC4]"></i> لوحة التحكم
            </button>
            <button onclick="showView('stats')" class="nav-btn w-full flex items-center gap-3 p-3 rounded-xl font-bold text-sm hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 transition">
                <i class="ph ph-chart-bar text-xl text-[#88B2AC]"></i> الإحصائيات
            </button>
            <button id="logsNavBtn" onclick="showView('logs')" class="nav-btn w-full flex items-center gap-3 p-3 rounded-xl font-bold text-sm hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 transition">
                <i class="ph ph-clock-clockwise text-xl text-purple-500"></i> سجل العمليات
            </button>
            
            <div class="pt-4 mt-4 border-t dark:border-slate-700">
                <span class="text-[10px] text-slate-400 font-bold px-2 uppercase">الإدارة</span>
                <button onclick="showView('halls')" id="btnHalls" class="w-full flex items-center gap-3 p-3 rounded-xl font-bold text-sm hover:bg-blue-50 dark:hover:bg-blue-900/30 text-[#6B9AC4] mt-2 transition">
                    <i class="ph ph-door text-xl"></i> إدارة القاعات
                </button>
                <button onclick="showView('users')" id="btnUsers" class="w-full flex items-center gap-3 p-3 rounded-xl font-bold text-sm hover:bg-blue-50 dark:hover:bg-blue-900/30 text-[#6B9AC4] transition">
                    <i class="ph ph-users text-xl"></i> إدارة الموظفين
                </button>
                <button onclick="openModal('resetModal')" id="btnReset" class="w-full flex items-center gap-3 p-3 rounded-xl font-bold text-sm hover:bg-red-50 dark:hover:bg-red-900/30 text-[#C97C7C] transition">
                    <i class="ph ph-arrow-counter-clockwise text-xl"></i> تصفير العدادات
                </button>
            </div>
        </nav>
        ...
        <div class="p-4 border-t dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
            ...
            <button onclick="location.reload()" class="w-full mt-3 py-2 bg-red-100 dark:bg-red-900/30 text-[#C97C7C] rounded-lg text-xs font-bold hover:bg-red-200 transition">تسجيل خروج</button>
        </div>
    </aside>

    <!-- Main Content -->
    <main class="flex-1 lg:mr-64 p-4 sm:p-6 lg:p-8 pt-20 lg:pt-8">
        <!-- KPIs - Responsive Grid -->
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 bg-white dark:bg-slate-800 rounded-xl shadow-sm p-3 mb-6 sm:mb-8 gap-3 sm:gap-4 border dark:border-slate-700" id="kpiContainer">
            <div class="text-center p-2 sm:p-3">
                <span class="block text-xs text-slate-400 mb-1">داخل المبنى</span>
                <span id="kpiIndoor" class="text-xl sm:text-2xl font-black text-[#6B9AC4]">0</span>
            </div>
            <div class="text-center p-2 sm:p-3 border-r dark:border-slate-700">
                <span class="block text-xs text-slate-400 mb-1">ينتظر خارجاً</span>
                <span id="kpiOutdoor" class="text-xl sm:text-2xl font-black text-[#E8A87C]">0</span>
            </div>
            <div class="text-center p-2 sm:p-3 border-r dark:border-slate-700">
                <span class="block text-xs text-slate-400 mb-1">الطاقة</span>
                <span id="kpiCapacity" class="text-xl sm:text-2xl font-black text-slate-600 dark:text-slate-300">0</span>
            </div>
            <div class="text-center p-2 sm:p-3 border-r dark:border-slate-700">
                <span class="block text-xs text-slate-400 mb-1">تمت خدمتهم</span>
                <span id="kpiServed" class="text-xl sm:text-2xl font-black text-[#88B2AC]">0</span>
            </div>
            <div class="text-center p-2 sm:p-3 border-r dark:border-slate-700">
                <span class="block text-xs text-slate-400 mb-1">الإشغال</span>
                <span id="kpiOccupancy" class="text-xl sm:text-2xl font-black text-[#B8A4C9]">0%</span>
            </div>
        </div>
...
<div id="incidentModal" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 overflow-y-auto">
    <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 w-full max-w-2xl modal-content my-8">
        <h3 class="font-bold text-xl mb-6 flex items-center gap-2">
            ...
            <span>رفع بلاغ جديد</span>
        </h3>
        
        <form id="incidentForm" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">القاعة</label>
                ...
                <div>
                    <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">نوع البلاغ</label>
                    <select id="incidentCategory" required class="w-full p-3 border-2 dark:border-slate-600 dark:bg-slate-700 dark:text-white rounded-xl font-bold focus:border-blue-500 outline-none transition">
                        <option value="CROWD">ازدحام شديد</option>
                        <option value="MAINTENANCE">صيانة / نظافة</option>
            ...
            <div>
                <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">الأولوية</label>
            ...
            <div>
                <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">تفاصيل البلاغ</label>
                <textarea id="incidentDescription" required rows="4" class="w-full p-3 border-2 dark:border-slate-600 dark:bg-slate-700 dark:text-white rounded-xl font-bold focus:border-blue-500 outline-none transition resize-none" placeholder="اكتب تفاصيل البلاغ هنا..."></textarea>
            </div>
            
            <div>
                <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">إرفاق صورة (اختياري)</label>
                <div class="upload-area rounded-xl p-6 cursor-pointer text-center" onclick="document.getElementById('incidentImage').click()">
                    ...
                    <p class="text-sm font-bold text-slate-500 dark:text-slate-400">اضغط لرفع صورة</p>
                ...
                <div id="imagePreviewContainer" class="hidden mt-3">
                    ...
                    <button type="button" onclick="removeImage()" class="mt-2 text-sm text-[#C97C7C] hover:text-red-700 font-bold">
                        <i class="ph ph-trash"></i> إزالة الصورة
                    </button>
                </div>
            </div>
        </form>
        ...
        <div class="flex gap-2 mt-6">
            <button onclick="closeModal('incidentModal')" class="flex-1 bg-slate-100 dark:bg-slate-700 p-3 rounded-xl font-bold hover:bg-slate-200 dark:hover:bg-slate-600 transition">إلغاء</button>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 w-full max-w-3xl modal-content my-8">
        ...
        <div class="mt-6 pt-6 border-t dark:border-slate-700">
            ...
            <div class="flex gap-2">
                <input type="text" id="commentInput" placeholder="أضف تعليق..." class="flex-1 p-3 border-2 dark:border-slate-600 dark:bg-slate-700 dark:text-white rounded-xl font-bold focus:border-blue-500 outline-none transition">
...
<div id="resetModal" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
    <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 w-full max-w-md modal-content">
        <h3 class="font-bold text-lg mb-4 flex items-center gap-2">
            <i class="ph ph-arrow-counter-clockwise text-[#6B9AC4]"></i> تصفير العدادات
        </h3>
        <div class="space-y-3">
            <label class="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-700 rounded-lg cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-600 transition">
                ...
                <div>
                    <p class="font-bold text-sm">تمت خدمتهم</p>
                    <p class="text-xs text-slate-500 dark:text-slate-400">تصفير عداد الزوار</p>
                </div>
            </label>
            <label class="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-700 rounded-lg cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-600 transition">
                <input type="checkbox" id="resetOutdoor" class="w-4 h-4">
                <div>
                    <p class="font-bold text-sm">قائمة الانتظار</p>
                    <p class="text-xs text-slate-500 dark:text-slate-400">تصفير الانتظار الخارجي</p>
                </div>
            </label>
            <label class="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-700 rounded-lg cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-600 transition">
                <input type="checkbox" id="resetRooms" class="w-4 h-4">
                <div>
                    <p class="font-bold text-sm">جميع القاعات</p>
                    <p class="text-xs text-slate-500 dark:text-slate-400">تصفير الحضور في القاعات</p>
                </div>
            </label>
        </div>
        <div class="flex gap-2 mt-4">
            <button onclick="closeModal('resetModal')" class="flex-1 bg-slate-100 dark:bg-slate-700 p-3 rounded-xl font-bold hover:bg-slate-200 dark:hover:bg-slate-600 transition">إلغاء</button>
            <button onclick="performReset()" class="flex-1 bg-[#6B9AC4] text-white p-3 rounded-xl font-bold hover:bg-[#5A89B3] transition">تصفير</button>
        </div>
    </div>
</div>
...
<div id="hallModal" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
    <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 w-full max-w-lg modal-content">
        <h3 class="font-bold text-xl mb-6 flex items-center gap-2">
            ...
            <span id="hallModalTitle">إضافة قاعة جديدة</span>
        </h3>
        <div class="space-y-4">
            <input type="hidden" id="hallId">
            <div>
                <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">اسم القاعة</label>
                <input type="text" id="hallName" required class="w-full p-3 border-2 dark:border-slate-600 dark:bg-slate-700 dark:text-white rounded-xl font-bold focus:border-blue-500 outline-none transition" placeholder="مثال: قاعة 1">
            </div>
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">السعة القصوى</label>
                ...
                <div>
                    ...
                    <select id="hallType" class="w-full p-3 border-2 dark:border-slate-600 dark:bg-slate-700 dark:text-white rounded-xl font-bold focus:border-blue-500 outline-none transition">
                        <option value="MAIN">قاعة رئيسية</option>
                        <option value="WAITING">منطقة انتظار</option>
            ...
            <div>
                <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">المسؤول</label>
            ...
            <div class="flex items-center gap-3 p-4 bg-slate-50 dark:bg-slate-700 rounded-xl">
                ...
                <div>
                    <p class="font-bold text-sm">القاعة مفعّلة</p>
                    <p class="text-xs text-slate-500 dark:text-slate-400">إظهار القاعة في لوحة التحكم</p>
                </div>
            </div>
        </div>
        <div class="flex gap-2 mt-6">
            <button onclick="closeModal('hallModal')" class="flex-1 bg-slate-100 dark:bg-slate-700 p-3 rounded-xl font-bold hover:bg-slate-200 dark:hover:bg-slate-600 transition">إلغاء</button>
            <button onclick="saveHall()" class="flex-1 bg-[#6B9AC4] text-white p-3 rounded-xl font-bold hover:bg-[#5A89B3] transition flex items-center justify-center gap-2">
                <i class="ph ph-floppy-disk text-xl"></i><span>حفظ</span>
            </button>
        </div>
    </div>
</div>
...
<div id="userModal" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
    <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 w-full max-w-lg modal-content">
        <h3 class="font-bold text-xl mb-6 flex items-center gap-2">
            ...
            <span id="userModalTitle">إضافة موظف جديد</span>
        </h3>
        <div class="space-y-4">
            ...
            <div>
                ...
                <input type="password" id="userPassword" required class="w-full p-3 border-2 dark:border-slate-600 dark:bg-slate-700 dark:text-white rounded-xl font-bold focus:border-blue-500 outline-none transition" placeholder="••••••">
            </div>
            <div>
                <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">الصلاحية</label>
                <select id="userRole" onchange="toggleRoomSelect()" class="w-full p-3 border-2 dark:border-slate-600 dark:bg-slate-700 dark:text-white rounded-xl font-bold focus:border-blue-500 outline-none transition">
                    ...
                    <option value="ORGANIZER_INDOOR">منظم - داخل المبنى</option>
                    <option value="ORGANIZER_OUTDOOR">مسؤول - الساحة الخارجية</option>
                    <option value="VIEWER">شاشة عرض (Viewer)</option>
                </select>
            </div>
            <div id="roomSelectContainer" class="hidden">
                <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">القاعة المسؤول عنها</label>
            ...
            <div>
                <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">الاسم الكامل (اختياري)</label>
                <input type="text" id="userFullName" class="w-full p-3 border-2 dark:border-slate-600 dark:bg-slate-700 dark:text-white rounded-xl font-bold focus:border-blue-500 outline-none transition" placeholder="محمد أحمد">
            </div>
        </div>
        <div class="flex gap-2 mt-6">
            <button onclick="closeModal('userModal')" class="flex-1 bg-slate-100 dark:bg-slate-700 p-3 rounded-xl font-bold hover:bg-slate-200 dark:hover:bg-slate-600 transition">إلغاء</button>
    ...
    try {
        ...
        if (d.exists() && d.data().pass === p) {
            ...
            if (currentUser.role === 'VIEWER') {
                ...
                navBtns.forEach(btn => {
                    if (btn.textContent.includes('لوحة التحكم') || 
                        btn.textContent.includes('البلاغات') || 
                        btn.textContent.includes('سجل العمليات')) {
    ...
    if (currentUser.role === 'ADMIN') {
        ...
        if (myRoom) {
            ...
            kpiContainer.innerHTML = `
                ...
                <div class="text-center px-4 border-r dark:border-slate-700">
                    <span class="block text-xs text-slate-400 mb-1">سعة قاعتي</span>
...
function renderIncidents() {
    ...
    const avgResponseTime = '15 دقيقة'; // Placeholder
    
    content.innerHTML = `
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-black">إدارة البلاغات</h2>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div class="bg-gradient-to-br from-[#C97C7C] to-[#B46D6D] p-5 rounded-2xl text-white shadow-lg">
            ...
            <p class="text-sm opacity-90">بلاغات جديدة</p>
        </div>
        ...
        <div class="bg-gradient-to-br from-[#88B2AC] to-[#7AA39D] p-5 rounded-2xl text-white shadow-lg">
            ...
            <p class="text-sm opacity-90">تم الحل</p>
        </div>
        <div class="bg-gradient-to-br from-[#B8A4C9] to-[#A895BA] p-5 rounded-2xl text-white shadow-lg">
            ...
            <p class="text-sm opacity-90">عاجلة نشطة</p>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 p-4 rounded-2xl border dark:border-slate-700 shadow-sm mb-6">
        <div class="flex flex-wrap gap-3">
            <select id="filterStatus" onchange="filterIncidents()" class="px-4 py-2 border dark:border-slate-600 dark:bg-slate-700 dark:text-white rounded-lg font-bold outline-none">
                ...
                <option value="PROGRESS">قيد المعالجة</option>
            ...
            <select id="filterCategory" onchange="filterIncidents()" class="px-4 py-2 border dark:border-slate-600 dark:bg-slate-700 dark:text-white rounded-lg font-bold outline-none">
                ...
                <option value="CROWD">ازدحام</option>
            ...
            <input type="text" id="filterSearch" placeholder="🔍 بحث..." onkeyup="filterIncidents()" class="flex-1 px-4 py-2 border dark:border-slate-600 dark:bg-slate-700 dark:text-white rounded-lg font-bold outline-none min-w-[200px]">
            
            <button onclick="exportIncidents()" class="px-4 py-2 bg-[#88B2AC] text-white rounded-lg font-bold hover:bg-green-700 transition">
                <i class="ph ph-download"></i> تصدير
            </button>
        </div>
    </div>
...
function displayIncidents(incidentsToShow) {
    ...
    if (incidentsToShow.length === 0) {
        grid.innerHTML = '<div class="col-span-full text-center py-20"><p class="text-2xl font-bold text-slate-400">لا توجد بلاغات</p></div>';
        return;
    }
    ...
    incidentsToShow.forEach(incident => {
        ...
        if (incident.category === 'CROWD') categoryText = 'ازدحام';
        else if (incident.category === 'MAINTENANCE') categoryText = 'صيانة';
        else if (incident.category === 'SECURITY') categoryText = 'أمن';
        else if (incident.category === 'TECHNICAL') categoryText = 'تقني';
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl border dark:border-slate-700 shadow-sm hover:shadow-md transition overflow-hidden cursor-pointer" onclick="viewIncident('${incident.id}')">
            ...
            <div class="p-4">
                <div class="flex justify-between items-start mb-3">
                    <div>
                        <h3 class="font-bold text-lg mb-1">${incident.roomName || 'منطقة عامة'}</h3>
                ...
                <div class="flex justify-between items-center pt-3 border-t dark:border-slate-700 text-xs">
                    <span class="text-slate-500 dark:text-slate-400">
                        <i class="ph ph-user-circle"></i> ${incident.reportedBy || 'غير محدد'}
                    </span>
                    <span class="text-slate-500 dark:text-slate-400">
                        <i class="ph ph-chat-circle-dots"></i> ${commentsCount} تعليق
                    </span>
                </div>
            </div>
        </div>
...
window.exportIncidents = () => {
    const csvContent = "data:text/csv;charset=utf-8," + "الوقت,القاعة,النوع,الأولوية,الحالة,التفاصيل,المبلغ\n" + reports.map(r => {
        const time = r.timestamp ? new Date(r.timestamp.seconds * 1000).toLocaleString('ar-SA') : 'الآن';
        const status = r.status === 'NEW' ? 'جديد' : r.status === 'PROGRESS' ? 'قيد المعالجة' : 'تم الحل';
        const priority = r.priority === 'HIGH' ? 'عاجل' : r.priority === 'MEDIUM' ? 'متوسط' : 'عادي';
    ...
    link.click();
    showToast('تم تصدير البلاغات', 'success');
};

// Create Incident Modal
window.openCreateIncidentModal = () => {
    document.getElementById('incidentHall').innerHTML = '<option value="">-- اختر القاعة --</option>' + 
...
window.submitIncident = async () => {
    ...
    showToast('جاري رفع البلاغ...', 'info');
    ...
    try {
        ...
        await logActivity('INCIDENT_CREATE', `رفع بلاغ: ${category} - ${priority}`, hallId);
        
        closeModal('incidentModal');
        showToast('تم رفع البلاغ بنجاح', 'success');
    } catch (e) {
        console.error('Error:', e);
        showToast('حدث خطأ أثناء رفع البلاغ', 'error');
    }
};
...
window.viewIncident = async (incidentId) => {
    ...
    let categoryText = incident.category;
    if (incident.category === 'CROWD') categoryText = 'ازدحام شديد';
    else if (incident.category === 'MAINTENANCE') categoryText = 'صيانة / نظافة';
    else if (incident.category === 'SECURITY') categoryText = 'أمن وسلامة';
    else if (incident.category === 'TECHNICAL') categoryText = 'مشكلة تقنية';
    else categoryText = 'أخرى';
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
            <label class="text-xs font-bold text-slate-500 dark:text-slate-400">الحالة</label>
            ${canManage ? `
                <select id="statusSelect" onchange="updateIncidentStatus('${incidentId}')" class="w-full mt-1 p-2 border dark:border-slate-600 dark:bg-slate-700 dark:text-white rounded-lg font-bold outline-none">
                    <option value="NEW" ${incident.status === 'NEW' ? 'selected' : ''}>جديد</option>
                    <option value="PROGRESS" ${incident.status === 'PROGRESS' ? 'selected' : ''}>قيد المعالجة</option>
                    <option value="RESOLVED" ${incident.status === 'RESOLVED' ? 'selected' : ''}>تم الحل</option>
                </select>
            ` : `
        ...
        <div>
            <label class="text-xs font-bold text-slate-500 dark:text-slate-400">الأولوية</label>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
        <div>
            <label class="text-xs font-bold text-slate-500 dark:text-slate-400">القاعة</label>
            <p class="mt-1 font-bold">${incident.roomName || 'منطقة عامة'}</p>
        </div>
        
        <div>
            <label class="text-xs font-bold text-slate-500 dark:text-slate-400">نوع البلاغ</label>
            <p class="mt-1 font-bold">${categoryText}</p>
        </div>
    </div>
    
    <div class="mt-4">
        <label class="text-xs font-bold text-slate-500 dark:text-slate-400">التفاصيل</label>
        <p class="mt-1 text-sm leading-relaxed p-4 bg-slate-50 dark:bg-slate-700 rounded-lg">${incident.desc}</p>
    </div>
    
    ${incident.imageUrl ? `
        <div class="mt-4">
            <label class="text-xs font-bold text-slate-500 dark:text-slate-400">الصورة المرفقة</label>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 pt-4 border-t dark:border-slate-700">
        <div>
            <label class="text-xs font-bold text-slate-500 dark:text-slate-400">المبلّغ</label>
            <p class="mt-1 font-bold text-sm">${incident.reportedBy || 'غير محدد'}</p>
        </div>
        
        <div>
            <label class="text-xs font-bold text-slate-500 dark:text-slate-400">وقت الإبلاغ</label>
    ...
    ${canManage ? `
        <div class="mt-4 pt-4 border-t dark:border-slate-700 flex gap-2">
            <button onclick="deleteIncident('${incidentId}')" class="flex-1 bg-[#C97C7C] text-white p-3 rounded-xl font-bold hover:bg-[#B46D6D] transition">
                <i class="ph ph-trash"></i> حذف البلاغ
            </button>
        </div>
    ` : ''}
    `;
...
function loadComments(incident) {
    ...
    if (!incident.comments || incident.comments.length === 0) {
        container.innerHTML = '<p class="text-sm text-slate-400 text-center py-4">لا توجد تعليقات</p>';
        return;
    }
...
window.updateIncidentStatus = async (incidentId) => {
    ...
    await logActivity('INCIDENT_UPDATE', `تحديث حالة البلاغ إلى: ${newStatus}`, incidentId);
...
window.deleteIncident = async (incidentId) => {
    ...
    await logActivity('INCIDENT_DELETE', 'حذف بلاغ', incidentId);
...
function renderDashboard() {
    ...
    if (currentUser.role === 'ORGANIZER_OUTDOOR' || currentUser.role === 'ADMIN') {
        ...
        <div class="bg-gradient-to-br from-[#E8A87C] to-[#D89A6D] p-6 rounded-2xl text-white shadow-lg mb-6 max-w-md">
            <h3 class="font-bold text-lg mb-4 flex items-center gap-2">
                <i class="ph ph-users text-2xl"></i> الساحة الخارجية
            </h3>
            ...
            <div class="flex justify-between items-start mb-3">
                ${currentUser.role !== 'VIEWER' ? `
                    <button onclick="togglePause('${r.id}', '${r.status || 'OPEN'}')" 
                        ...
                        <i class="ph ${isPaused ? 'ph-play' : 'ph-pause'}"></i> ${isPaused ? 'تفعيل' : 'إيقاف'}
                    </button>
                ` : '<div></div>'}
...
function renderHalls() {
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div class="bg-gradient-to-br from-[#6B9AC4] to-[#5A89B3] p-5 rounded-2xl text-white">
            ...
            <p class="text-sm opacity-90">إجمالي القاعات</p>
        </div>
        <div class="bg-gradient-to-br from-[#88B2AC] to-[#7AA39D] p-5 rounded-2xl text-white">
            ...
            <p class="text-sm opacity-90">قاعات مفعّلة</p>
        </div>
        <div class="bg-gradient-to-br from-[#E8A87C] to-[#D89A6D] p-5 rounded-2xl text-white">
            ...
            <h3 class="text-3xl font-black">${rooms.filter(r => !r.active).length}</h3>
            <p class="text-sm opacity-90">قاعات معطّلة</p>
        </div>
        <div class="bg-gradient-to-br from-[#B8A4C9] to-[#A895BA] p-5 rounded-2xl text-white">
            ...
            <p class="text-sm opacity-90">السعة الإجمالية</p>
        </div>
    </div>
            ...
            <div class="bg-white dark:bg-slate-800 p-5 rounded-2xl border dark:border-slate-700 shadow-sm hover:shadow-md transition ${!r.active ? 'opacity-60' : ''}">
                <div class="flex justify-between items-start mb-3">
                    <div>
                        ...
                        <span class="text-xs px-2 py-1 rounded ${r.active ? 'bg-green-100 text-[#88B2AC] dark:bg-green-900/30' : 'bg-slate-100 text-slate-600 dark:bg-slate-700'}">
                            ${r.active ? '✓ مفعّلة' : '✗ معطّلة'}
                        </span>
                    </div>
                    <div class="flex gap-2">
                        <button onclick="editHall('${r.id}')" class="p-2 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-lg transition" title="تعديل">
                            <i class="ph ph-pencil-simple text-[#6B9AC4] dark:text-blue-400"></i>
                        </button>
                        <button onclick="deleteHall('${r.id}', '${r.name}')" class="p-2 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg transition" title="حذف">
                ...
                <div class="space-y-3">
                    <div class="flex justify-between text-sm">
                        <span class="text-slate-500 dark:text-slate-400">السعة القصوى</span>
                        <span class="font-bold">${r.capacity} شخص</span>
                    </div>
                    <div class="flex justify-between text-sm">
                        <span class="text-slate-500 dark:text-slate-400">الحضور الحالي</span>
                        <span class="font-bold">${r.current} شخص</span>
                    </div>
                    <div>
                        <div class="flex justify-between text-xs mb-1">
                            <span class="text-slate-500 dark:text-slate-400">نسبة الإشغال</span>
                    ...
                    <div class="flex justify-between text-sm pt-2 border-t dark:border-slate-700">
                        <span class="text-slate-500 dark:text-slate-400">المسؤول</span>
                        <span class="font-bold">${supervisor ? supervisor.fullName || supervisor.id : 'غير محدد'}</span>
                    </div>
                </div>
            </div>
            `;
...
window.saveHall = async () => {
    ...
    await b.commit();
    await logActivity(hallId ? 'HALL_EDIT' : 'HALL_ADD', `${hallId ? 'تعديل' : 'إضافة'} قاعة: ${hallName}`);
    closeModal('hallModal');
    showToast(hallId ? 'تم تحديث القاعة' : 'تم إضافة القاعة', 'success');
};

window.deleteHall = async (hallId, hallName) => {
    const result = await Swal.fire({
        title: 'هل أنت متأكد؟',
        text: `سيتم حذف القاعة "${hallName}" نهائياً`,
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#ef4444',
        cancelButtonColor: '#64748b',
    ...
    await b.commit();
    await logActivity('HALL_DELETE', `حذف قاعة: ${hallName}`);
...
function renderUsers() {
    ...
    <div class="flex justify-between items-center mb-6">
        ...
        <button onclick="openAddUserModal()" class="px-4 py-2 bg-[#6B9AC4] text-white rounded-xl font-bold hover:bg-[#5A89B3] transition flex items-center gap-2">
            <i class="ph ph-user-plus text-xl"></i> إضافة موظف جديد
        </button>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-gradient-to-br from-slate-500 to-slate-600 p-5 rounded-2xl text-white">
            ...
            <p class="text-sm opacity-90">شاشات عرض</p>
        </div>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead class="bg-slate-50 dark:bg-slate-900">
                    <tr class="text-right">
                        <th class="p-4 font-bold">اسم المستخدم</th>
                        <th class="p-4 font-bold">الاسم الكامل</th>
                        <th class="p-4 font-bold">الصلاحية</th>
                        <th class="p-4 font-bold">القاعة</th>
                ...
                <tbody class="divide-y dark:divide-slate-700">
                    ${users.map(u => {
                        let roleText = '';
                        let roleColor = '';
                        if (u.role === 'ADMIN') { roleText = 'مشرف'; roleColor = 'bg-purple-100 text-[#B8A4C9] dark:bg-purple-900/30'; }
                        else if (u.role === 'ORGANIZER_INDOOR') { roleText = 'منظم - داخلي'; roleColor = 'bg-blue-100 text-[#6B9AC4] dark:bg-blue-900/30'; }
                        else if (u.role === 'ORGANIZER_OUTDOOR') { roleText = 'مسؤول - خارجي'; roleColor = 'bg-orange-100 text-[#E8A87C] dark:bg-orange-900/30'; }
                        else if (u.role === 'VIEWER') { roleText = 'شاشة عرض'; roleColor = 'bg-slate-100 text-slate-600 dark:bg-slate-700'; }
                        ...
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition">
                            ...
                            <td class="p-4">
                                <div class="flex gap-2">
                                    <button onclick="editUser('${u.id}')" class="p-2 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-lg transition" title="تعديل">
                                        <i class="ph ph-pencil-simple text-[#6B9AC4] dark:text-blue-400"></i>
                                    </button>
                                    ${u.role !== 'ADMIN' ? `
                                        <button onclick="deleteUser('${u.id}')" class="p-2 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg transition" title="حذف">
                                            <i class="ph ph-trash text-[#C97C7C] dark:text-red-400"></i>
                                        </button>
                                    ` : ''}
                                </div>
...
window.openAddUserModal = () => {
    document.getElementById('userModalTitle').innerText = 'إضافة موظف جديد';
...
window.editUser = async (userId) => {
    ...
    if (!user) return;
    document.getElementById('userModalTitle').innerText = 'تعديل الموظف';
...
window.saveUser = async () => {
    ...
    if (userRole === 'ORGANIZER_INDOOR' && !userRoom) {
        showToast('يرجى اختيار القاعة للمنظم الداخلي', 'error');
        return;
    }
    const userData = {
        pass: userPassword,
    ...
    await logActivity(userId ? 'USER_EDIT' : 'USER_ADD', `${userId ? 'تعديل' : 'إضافة'} موظف: ${userName}`);
    closeModal('userModal');
    showToast(userId ? 'تم تحديث الموظف' : 'تم إضافة الموظف', 'success');
...
window.deleteUser = async (userId) => {
    ...
    const result = await Swal.fire({
        title: 'هل أنت متأكد؟',
        text: `سيتم حذف المستخدم "${user.fullName || userId}" نهائياً`,
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#ef4444',
        cancelButtonColor: '#64748b',
    ...
    await logActivity('USER_DELETE', `حذف موظف: ${userId}`);
...
function renderStats() {
    ...
    content.innerHTML = `
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-black">الإحصائيات المتقدمة</h2>
        <div class="flex gap-2">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
        <div class="bg-gradient-to-br from-[#6B9AC4] to-[#5A89B3] p-6 rounded-2xl text-white shadow-lg">
            ...
            <p class="text-sm opacity-90">داخل المبنى</p>
        </div>
        <div class="bg-gradient-to-br from-[#E8A87C] to-[#D89A6D] p-6 rounded-2xl text-white shadow-lg">
            ...
            <p class="text-sm opacity-90">ينتظر خارجاً</p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div class="bg-white dark:bg-slate-800 p-6 rounded-2xl border dark:border-slate-700 shadow-sm">
            <h3 class="font-bold text-lg mb-4 flex items-center gap-2 text-[#88B2AC]">
                <i class="ph ph-trophy text-2xl"></i> أكثر قاعة ازدحاماً
            </h3>
            ${mostBusy ? `
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-2xl font-black mb-1">${mostBusy.name}</p>
                        <p class="text-sm text-slate-500">${mostBusy.current} / ${mostBusy.capacity} زائر</p>
                    </div>
                    <div class="text-4xl font-black text-[#88B2AC]">
                        ${Math.round((mostBusy.current / mostBusy.capacity) * 100)}%
                    </div>
                </div>
            ` : '<p class="text-slate-400">لا توجد بيانات</p>'}
        </div>
        <div class="bg-white dark:bg-slate-800 p-6 rounded-2xl border dark:border-slate-700 shadow-sm">
            <h3 class="font-bold text-lg mb-4 flex items-center gap-2 text-[#6B9AC4]">
                <i class="ph ph-clock text-2xl"></i> أقل قاعة استخداماً
            </h3>
            ${leastBusy ? `
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-2xl font-black mb-1">${leastBusy.name}</p>
                        <p class="text-sm text-slate-500">${leastBusy.current} / ${leastBusy.capacity} زائر</p>
                    </div>
    ...
    <div class="bg-white dark:bg-slate-800 p-6 rounded-2xl border dark:border-slate-700 shadow-sm">
        <h3 class="font-bold text-lg mb-4">تفاصيل القاعات</h3>
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead class="bg-slate-50 dark:bg-slate-900">
                    <tr class="text-right">
                        <th class="p-3 font-bold">القاعة</th>
                        <th class="p-3 font-bold">الحضور</th>
                        <th class="p-3 font-bold">السعة</th>
                        <th class="p-3 font-bold">الإشغال</th>
                ...
                <tbody class="divide-y dark:divide-slate-700">
                    ${activeRooms.map(r => {
                        ...
                        const status = r.status === 'PAUSED' ? 'مغلق' : (ratio >= 100 ? 'ممتلئ' : ratio >= 80 ? 'شبه ممتلئ' : 'متاح');
...
window.exportStatsCSV = () => {
    const activeRooms = rooms.filter(r => r.active);
    const csvContent = "data:text/csv;charset=utf-8," + "القاعة,الحضور,السعة,نسبة الإشغال,الحالة\n" + activeRooms.map(r => {
        const ratio = Math.round((r.current / r.capacity) * 100);
        const status = r.status === 'PAUSED' ? 'مغلق' : (ratio >= 100 ? 'ممتلئ' : ratio >= 80 ? 'شبه ممتلئ' : 'متاح');
    ...
    link.click();
    showToast('تم تصدير الإحصائيات', 'success');
};
...
function renderLogs() {
    ...
    content.innerHTML = `
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-black">سجل العمليات</h2>
    ...
    <div class="bg-white dark:bg-slate-800 p-4 rounded-2xl border dark:border-slate-700 mb-6">
        <div class="flex gap-3">
            <input type="text" id="logSearch" placeholder="🔍 بحث..." onkeyup="filterLogs()" class="flex-1 px-4 py-2 border dark:border-slate-600 dark:bg-slate-700 rounded-lg">
            <select id="logFilter" onchange="filterLogs()" class="px-4 py-2 border dark:border-slate-600 dark:bg-slate-700 rounded-lg">
                ...
                <option value="MANUAL_SET">تعديل يدوي</option>
                <option value="PAUSE">إيقاف</option>
                <option value="RESUME">تفعيل</option>
                <option value="INCIDENT_CREATE">رفع بلاغ</option>
                <option value="INCIDENT_UPDATE">تحديث بلاغ</option>
            </select>
        </div>
    </div>
            ...
            <table class="w-full text-sm">
                <thead class="bg-slate-50 dark:bg-slate-900 sticky top-0">
                    <tr class="text-right">
                        ...
                        <th class="p-3 font-bold">العملية</th>
                        <th class="p-3 font-bold">القاعة</th>
...
function displayLogs(logsToShow) {
    ...
    tbody.innerHTML = logsToShow.length === 0 ? '<tr><td colspan="5" class="p-8 text-center text-slate-400">لا توجد سجلات</td></tr>' : '';
    ...
    logsToShow.forEach(log => {
        ...
        if (log.action === 'ENTRY') { actionText = '✅ دخول'; actionColor = 'text-[#88B2AC]'; }
        else if (log.action === 'EXIT') { actionText = '🚪 خروج'; actionColor = 'text-[#6B9AC4]'; }
        else if (log.action === 'MANUAL_SET') { actionText = '✏️ تعديل يدوي'; actionColor = 'text-[#B8A4C9]'; }
        else if (log.action === 'PAUSE') { actionText = '⏸️ إيقاف'; actionColor = 'text-[#E8A87C]'; }
        else if (log.action === 'RESUME') { actionText = '▶️ تفعيل'; actionColor = 'text-[#88B2AC]'; }
        else if (log.action === 'INCIDENT_CREATE') { actionText = '🚨 رفع بلاغ'; actionColor = 'text-[#C97C7C]'; }
        else if (log.action === 'INCIDENT_UPDATE') { actionText = '🔄 تحديث بلاغ'; actionColor = 'text-[#E8A87C]'; }
...
window.exportLogs = () => {
    const csvContent = "data:text/csv;charset=utf-8," + "الوقت,الموظف,العملية,القاعة,التفاصيل\n" + logs.map(log => {
        const time = log.timestamp ? new Date(log.timestamp.seconds * 1000).toLocaleString('ar-SA') : 'الآن';
...
window.adjustOutdoor = async (dir) => {
    if (dir === -1 && globalStats.outdoor_queue <= 0) {
        showToast('لا يوجد زوار في الانتظار', 'warning');
        return;
    }
    await updateDoc(doc(db, "settings", "global_config"), { outdoor_queue: increment(dir) });
    await logActivity(dir === 1 ? 'OUTDOOR_ADD' : 'OUTDOOR_REMOVE', dir === 1 ? 'إضافة زائر للانتظار' : 'خصم زائر من الانتظار');
    showToast(dir === 1 ? 'تمت الإضافة' : 'تم الخصم', 'success');
};
...
window.setOutdoorManual = async () => {
    ...
    await logActivity('MANUAL_SET', `تعديل يدوي: الانتظار من ${oldVal} إلى ${newVal}`);
...
window.adjustRoom = async (id, dir) => {
    ...
    if (dir === 1) {
        if (r.current >= r.capacity) { showToast('القاعة ممتلئة', 'warning'); return; }
        if (globalStats.outdoor_queue <= 0) { showToast('لا يوجد زوار في الانتظار', 'warning'); return; }
        const b = writeBatch(db);
        b.update(doc(db, "rooms", id), { current: increment(1) });
        b.update(doc(db, "settings", "global_config"), { outdoor_queue: increment(-1) });
        await b.commit();
        await logActivity('ENTRY', 'دخول زائر', id);
        showToast('تم إضافة دخول', 'success');
    } else {
        ...
        await b.commit();
        await logActivity('EXIT', 'خروج زائر', id);
        showToast('تم تسجيل خروج', 'success');
    }
};
...
window.setRoomManual = async (id) => {
    ...
    const r = rooms.find(x => x.id === id);
    if (newVal < 0 || newVal > r.capacity) { showToast('قيمة غير صحيحة', 'error'); document.getElementById(`room_${id}`).value = r.current; return; }
    const diff = newVal - r.current;
    if (diff === 0) return;
    const b = writeBatch(db);
    if (diff > 0) {
        if (globalStats.outdoor_queue < diff) { showToast('عدد الانتظار الخارجي غير كافي', 'error'); document.getElementById(`room_${id}`).value = r.current; return; }
        b.update(doc(db, "rooms", id), { current: newVal });
        b.update(doc(db, "settings", "global_config"), { outdoor_queue: increment(-diff) });
        await b.commit();
        await logActivity('MANUAL_SET', `تعديل يدوي: ${r.name} من ${r.current} إلى ${newVal} (+${diff} من الانتظار)`, id);
        showToast(`تم إضافة ${diff} من الانتظار الخارجي`, 'success');
    } else {
        b.update(doc(db, "rooms", id), { current: newVal });
        b.update(doc(db, "settings", "global_config"), { served_count: increment(Math.abs(diff)) });
        await b.commit();
        await logActivity('MANUAL_SET', `تعديل يدوي: ${r.name} من ${r.current} إلى ${newVal} (${Math.abs(diff)} تمت خدمتهم)`, id);
        showToast(`تم نقل ${Math.abs(diff)} إلى "تمت خدمتهم"`, 'success');
    }
};
...
window.togglePause = async (id, currentStatus) => {
    ...
    await logActivity(newStatus === 'PAUSED' ? 'PAUSE' : 'RESUME', newStatus === 'PAUSED' ? 'إيقاف القاعة' : 'تفعيل القاعة', id);
    showToast(newStatus === 'PAUSED' ? 'تم الإيقاف' : 'تم التفعيل', 'success');
};
...
window.performReset = async () => {
    ...
    if (!resetServed && !resetOutdoor && !resetRooms) { showToast('اختر عنصر واحد على الأقل', 'warning'); return; }
    if (!confirm('تأكيد التصفير؟')) return;
    const b = writeBatch(db);
    let details = [];
    if (resetServed) { b.update(doc(db, "settings", "global_config"), { served_count: 0 }); details.push('تمت خدمتهم'); }
    if (resetOutdoor) { b.update(doc(db, "settings", "global_config"), { outdoor_queue: 0 }); details.push('قائمة الانتظار'); }
    if (resetRooms) { rooms.forEach(r => b.update(doc(db, "rooms", r.id), { current: 0 })); details.push('جميع القاعات'); }
    await b.commit();
    await logActivity('RESET', `تصفير: ${details.join(', ')}`);
    closeModal('resetModal');
    showToast('تم التصفير بنجاح', 'success');
};

window.systemReset = async () => {
    if (!confirm('هل أنت متأكد؟ سيتم حذف جميع البيانات!')) return;
    const b = writeBatch(db);
    b.set(doc(db, "users", "admin"), { pass: "1234", role: "ADMIN", fullName: "مدير النظام" });
    b.set(doc(db, "settings", "global_config"), { served_count: 0, outdoor_queue: 0 });
    for (let i = 1; i <= 20; i++) {
        b.set(doc(db, "rooms", `hall_${i}`), { id: `hall_${i}`, name: `قاعة ${i}`, type: i <= 10 ? 'MAIN' : 'WAITING', capacity: 100, current: 0, active: i <= 5, status: 'OPEN' });
    }
    await b.commit();
